...
//   - checkChannels(): query BOTH dist-tags at once —
//                    `latest` (官方正式版) and `alpha` (Alpha 预览版) — and
//                    report each channel's exact version or its own error, so
//                    one failing channel never hides the other
...
class VersionUpdateService extends TypertRemoteService {
   ...
   *   stable → dist-tag `latest` (官方正式版)
   *   alpha  → dist-tag `alpha`  (Alpha 预览版, published pre-release line)
   * The two lookups never throw and never fail together: a channel that errors
   * reports { version: null, error } while the other still carries its version.
   * `checkedAt` is set only when at least one channel actually resolved.
   */
  async checkChannels() {
    ...
    return {
      ...
      channels: {
        stable: { tag: 'stable', version: stable.version, error: stable.error },
        alpha: { tag: 'alpha', version: alpha.version, error: alpha.error }
      }
    }
  }
