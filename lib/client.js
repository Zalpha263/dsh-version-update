...
`;
		...
		function VersionUpdateSection(props) {
			...
			const showUpdate = phase === 'done' && latest !== null && info !== null && latest !== (info.localVersion || '');
			// info 加载失败时不能误报"已是最新"——无本地版本可比对
			const showUpToDate = phase === 'done' && latest !== null && info !== null && latest === (info.localVersion || '');
			...
			return React.createElement('div', { className: 'vu-section' },
				...
				React.createElement('div', { className: 'vu-card' },
					...
					phase === 'done' && showUpdate && React.createElement('div', { className: 'vu-card', style: { borderColor: 'var(--dsw-alias-state-warn-primary)' } },
						React.createElement('p', { className: 'vu-status vu-status-update' }, '发现新版本 v' + latest + '（当前 v' + (info.localVersion || '未知') + '）'),
