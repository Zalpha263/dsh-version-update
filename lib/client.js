...
`;
		...
		function VersionUpdateSection(props) {
			...
			const promptText = () => {
				...
				return '请帮我升级 dsh（DeepSeek Harness）到版本 ' + target + '：\n' +
					'1. 当前版本：' + local + '，安装目标：' + target + '\n' +
					'2. 官方更新方式：执行 npm install -g @deepseek-ai/dsh@' + target + '\n' +
					'3. 升级完成后运行 dsh --version 验证输出为 ' + target + '\n' +
					'4. 验证后提醒我彻底重启 dsh web（关掉旧 node 进程再启动）使新版本生效\n' +
					'注意：必须使用上面的精确版本号（不要用 @latest，它解析的是 npm 的 latest 标签，可能静默装到别的版本甚至降级）；' +
					'升级只替换全局安装的 @deepseek-ai/dsh 包，不影响 $DSH_HOME（~/.dsh）下的配置、会话与插件数据';
			};
			...
			return React.createElement('div', { className: 'vu-section' },
				...
				React.createElement('div', { className: 'vu-card' },
					...
					phase === 'done' && showUpToDate && React.createElement('p', { className: 'vu-status vu-status-ok' }, '当前已是最新版本 v' + localVersion),
